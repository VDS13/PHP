	...
	public function __construct()
	{
		...
		for ($i = 0; $i < Game::OBSTACLES_COUNT; $i++)
		{
			$this->_obstacles[] = new Obstacle(
				random_int(self::PARKING_OFFSET, self::MAP_WIDTH - self::PARKING_OFFSET - 1),
				random_int(0, self::MAP_HEIGHT - 1),
				1, 1, null, "obstacle", $this->_map);
		}
		$this->players = array();
		$this->players[0] = new Player('Player 1');
		$this->players[1] = new Player('Player 2');
		$this->players[0]->add_ship(new Destroyer(0,0, $this->_map, null));
		$this->players[1]->add_ship(new Destroyer(0,0, $this->_map, null));
